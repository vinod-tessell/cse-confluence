...
function runHealth(DATA) {
  ...
  // Update score bar in health panel
...
def build_customer_html(cust, data):
    ...
    if hist:
        ...
        # Chart init is called from window.onload via initChart() below.
        # We define the data here; the actual Chart() call lives in the script block
        # so it only runs after Chart.js CDN has finished loading.
        timeseries_js = f"""
var CHART_DATA={{
  labels:{chart_labels},
  open:{chart_data},
  ...
  <div class="grid2">
    <div>
      <div class="ai-panel" style="padding:0;overflow:hidden">
        ...
        <!-- ── Top strip: eyebrow + title + score bar ──────────────────────── -->
        ...
        <!-- ── Row 2: Findings+Actions | Customer Signals ───────────────────── -->
...
const DATA={data_js};
{HEALTH_JS}
window.onload=()=>{{try{{runHealth(DATA);}}catch(e){{console.error('runHealth error:',e);}}try{{buildHealthDrawer(DATA);}}catch(e){{console.error('buildHealthDrawer error:',e);}}try{{initChart();}}catch(e){{console.error('initChart error:',e);}}}};
